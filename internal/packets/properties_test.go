...
// TestPropertiesBinaryDataDeepCopy verifies that binary fields like CorrelationData
// and AuthenticationData are deep-copied during decoding. This is critical when
// using pooled buffers that may be reused immediately after decoding.
func TestPropertiesBinaryDataDeepCopy(t *testing.T) {
	correlationData := []byte("original-correlation-data")
	authData := []byte("original-auth-data")
	...
	// Create a buffer and copy encoded data into it to simulate a pooled buffer.
	buffer := make([]byte, len(encoded))
	copy(buffer, encoded)

	// Decode from the buffer
	decoded, _, err := decodeProperties(buffer)
	if err != nil {
		t.Fatalf("failed to decode: %v", err)
	}

	// Verify initial values
	if !reflect.DeepEqual(decoded.CorrelationData, correlationData) {
		t.Errorf("CorrelationData mismatch before corruption")
	}
	if !reflect.DeepEqual(decoded.AuthenticationData, authData) {
		t.Errorf("AuthenticationData mismatch before corruption")
	}

	// Simulate buffer corruption/reuse by overwriting the source buffer.
	for i := range buffer {
		buffer[i] = 0xAA
	}

	// Verify that decoded properties remain UNCHANGED.
	if !reflect.DeepEqual(decoded.CorrelationData, correlationData) {
		t.Errorf("CorrelationData was corrupted by buffer reuse! Deep copy failed.\nGot:  %02x\nWant: %02x",
			decoded.CorrelationData, correlationData)
	}
	if !reflect.DeepEqual(decoded.AuthenticationData, authData) {
		t.Errorf("AuthenticationData was corrupted by buffer reuse! Deep copy failed.\nGot:  %02x\nWant: %02x",
			decoded.AuthenticationData, authData)
	}
}
